...
async function importCopilotCli(sourceDir) {
  ...
  if (fs2.existsSync(instrDir)) {
    const files = await fg2("**/*.instructions.md", { cwd: instrDir, absolute: true });
    for (const filePath of files.sort()) {
  ...
  if (fs2.existsSync(promptsDir)) {
    const files = await fg2("**/*.prompt.md", { cwd: promptsDir, absolute: true });
    for (const filePath of files.sort()) {
      const raw = fs2.readFileSync(filePath, "utf8");
      const { content } = safeMatter(raw);
...
async function importCursor(sourceDir) {
  ...
  const files = await fg3("**/*.{mdc,md}", { cwd: rulesDir, absolute: true });
  for (const filePath of files.sort()) {
...
async function importClaudeCode(sourceDir) {
  ...
  if (fs4.existsSync(rulesDir)) {
    const files = await fg4("**/*.md", { cwd: rulesDir, absolute: true });
    for (const filePath of files.sort()) {
  ...
  if (fs4.existsSync(agentsDir)) {
    const files = await fg4("**/*.md", { cwd: agentsDir, absolute: true });
    for (const filePath of files.sort()) {
      const raw = fs4.readFileSync(filePath, "utf8");
...
async function importGeminiCli(sourceDir) {
  ...
  if (fs5.existsSync(instrDir)) {
    const files = await fg5("**/*.md", { cwd: instrDir, absolute: true });
    for (const filePath of files.sort()) {
...
async function importAntigravity(sourceDir) {
  ...
  const files = await fg6("**/*.md", { cwd: rulesDir, absolute: true });
  for (const filePath of files.sort()) {
...
async function importCodex(sourceDir) {
  ...
  if (fs7.existsSync(instrDir)) {
    const files = await fg7("**/*.md", { cwd: instrDir, absolute: true });
    for (const filePath of files.sort()) {
  ...
  if (fs7.existsSync(agentsDir)) {
    const files = await fg7("**/*.toml", { cwd: agentsDir, absolute: true });
    for (const filePath of files.sort()) {
...
async function importWindsurf(sourceDir) {
  ...
  const files = await fg8("**/*.md", { cwd: rulesDir, absolute: true });
  for (const filePath of files.sort()) {
...
async function importWindsurfCli(sourceDir) {
  ...
  const files = await fg9("**/*.md", { cwd: rulesDir, absolute: true });
  for (const filePath of files.sort()) {
...
async function importCline(sourceDir) {
  ...
  const files = await fg10("**/*.{md,txt}", { cwd: rulesDir, absolute: true });
  for (const filePath of files.sort()) {
...
async function loadConfig(configDir, overrides) {
  ...
  if (fs12.existsSync(configPath)) {
    raw = yaml2.load(fs12.readFileSync(configPath, "utf8")) ?? {};
  }
...
var GlobalToolConfigSchema = z2.object({
   ...
   */
  plugins: z2.array(z2.string()).default([...BUILT_IN_TARGETS])
});
...
async function loadGlobalConfig() {
  ...
  if (fs13.existsSync(configPath)) {
    raw = yaml3.load(fs13.readFileSync(configPath, "utf8")) ?? {};
  }
...
async function parseInstructions(configDir) {
  ...
  const filePaths = await fg11("**/*.md", { cwd: dir, absolute: true });
  filePaths.sort();
...
async function parseAgents(configDir) {
  ...
  const filePaths = await fg12("**/*.md", { cwd: dir, absolute: true });
  filePaths.sort();
  return filePaths.map((filePath) => {
...
async function parseCommands(configDir) {
  ...
  const filePaths = await fg13("**/*.md", { cwd: dir, absolute: true });
  filePaths.sort();
  return filePaths.map((filePath) => {
...
var HookSchema = z3.object({
  name: z3.string(),
  event: z3.enum(HOOK_EVENTS),
  matcher: z3.string().optional(),
  type: z3.enum(["command", "http", "prompt", "agent"]).default("command"),
  command: z3.string().optional(),
  timeout: z3.number().optional(),
  blocking: z3.boolean().optional(),
  async: z3.boolean().optional(),
  targets: z3.array(z3.string()).optional(),
  excludedTargets: z3.array(z3.string()).optional()
});
var HooksYamlSchema = z3.object({
  hooks: z3.array(HookSchema).default([])
});
function parseHooks(configDir) {
  const hooksFile = path18.join(configDir, "hooks", "hooks.yaml");
  if (!fs18.existsSync(hooksFile)) return [];
  const raw = yaml4.load(fs18.readFileSync(hooksFile, "utf8"));
...
export {
  BUILT_IN_TARGETS,
  PluginRegistry as GeneratorRegistry,
  PluginRegistry,
  clearHashCache,
  computeDiff,
  deduplicateOutputs,
