...
Graph and matrix writers render the supplied navigation tree with indentation and permanently visible groups. Diagram image export chooses the PNG background from the active light or dark viewer canvas. The graph toolbar can snap all diagram nodes, or the descendants of a selected expanded group, to a configurable 5-unit grid increment.
