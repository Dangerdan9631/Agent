...
- Q: Where is the local CLI installed? → A: `.spec-n-roll/cli/bin/spec-n-roll` (with `spec-n-roll.cmd` on Windows alongside it).
- Q: What extension hook events are supported? → A: Add `before_clarify`/`after_clarify` and `before_analyze`/`after_analyze`; `/spec-n-roll` has no dedicated hooks — it fires the underlying step's hooks.
...
- Q: Which agents does v1 support? → A: Extension-only agents — no hard-coded registry. Cursor, claude-code, copilot, and codex ship as out-of-the-box bundled extensions.
...
A developer initializes spec-n-roll on a new or existing project and enables one or more agent extensions (cursor, claude-code, copilot, and codex ship out-of-the-box as bundled extensions). The toolkit generates agent skills in `.agents/`, canonical rules in `.spec-n-roll/AGENTS.md`, and thin per-agent pointer files in native locations so any team member can immediately start a spec-driven workflow with their preferred agent, with no manual cross-agent setup.
...
- **FR-001**: The toolkit MUST support configuration of one or more AI coding agents during initialization via extensions only — no hard-coded agent registry. Cursor, claude-code, copilot, and codex MUST ship as out-of-the-box bundled extensions at `.spec-n-roll/bundled-extensions/{id}/`, referenced by default in `workflow.config.json`. The toolkit MUST generate skills in `.agents/`, canonical rules in `.spec-n-roll/AGENTS.md`, and thin per-agent pointer files in native locations. The toolkit MUST NOT verify that selected agents are installed.
...
- **FR-015**: The toolkit MUST provide a documented extension mechanism that supports registering custom workflow steps, hooks, triage logic (within `specify` or as replaceable built-in behavior), and workflow variant definitions in project configuration files. Extension `entrypoint` values MUST be project-relative paths to JS/TS modules exporting a standard handler, invoked in-process via `import()`.
...
- **FR-023**: The toolkit MUST provide a TypeScript CLI application as the primary interface for toolkit management. The CLI MUST support at minimum: initial project setup, version update, and configuration modification (enabling agent extensions, changing script variants). The CLI MUST be the only supported mechanism for applying toolkit updates. During initialization, the CLI MUST install a local copy at `.spec-n-roll/cli/bin/spec-n-roll` (with `spec-n-roll.cmd` on Windows). On every invocation, the globally installed CLI MUST delegate to the local copy when found; `--global` bypasses local delegation.
...
- **Agent Configuration**: Represents a configured AI coding agent delivered via an extension (bundled or user-installed). Skills live in `.agents/`; canonical rules live in `.spec-n-roll/AGENTS.md` with thin per-agent pointer files in native locations.
- **Living Specification**: A Cucumber Gherkin `.feature` file at `living-specs/{kebab-case-domain}.feature` representing current accepted behavior of one domain area. Agent infers domain semantically and auto-selects or creates the target file; planned changes appear in `plan.md`/`tasks.md` before implementation. Tagged with `@spec-n-roll-{numeric-id}`.
...
- **Extension**: A registered, self-contained module that provides a custom implementation of one or more workflow steps or hooks, invoked in-process via Node `import()` of a relative JS/TS module exporting a standard handler. Each extension manifest MUST declare the specific toolkit version it was designed for. This declaration is used at upgrade time to surface compatibility warnings — it does not block execution.
- **Triage (embedded in specify)**: Built-in logic at the start of the `specify` step that evaluates the feature description and selects a workflow tier variant (papercut/quick/full). Replaceable via extensions targeting the specify phase. Not a separate workflow step in tier definitions.
...
- **File Ownership Class**: Toolkit-owned: `.spec-n-roll/` (except `config/`) and `.agents/`. User-owned: `.spec-n-roll/config/`, `specs/`, `living-specs/`. Documentation in the toolkit repository is excluded.
...
- Agents are configured only via extensions; cursor, claude-code, copilot, and codex ship as bundled extensions. The toolkit does not verify that selected agents are installed.
