...
| **Full CLI** | `.spec-n-roll/cli/bin/spec-n-roll` (local) or with dispatcher (global fallback / `--global`) | Argument parsing, subcommands, Ink UI, update/config; bare `spec-n-roll` → interactive Ink; `spec-n-roll <subcommand>` → non-interactive sync exit |
...
Each agent extension manifest declares `agentSetup.mcpConfig.targets[]` (project-relative paths + format) and `agentSetup.mcpConfig.serverId` (stable merge key). Merge is idempotent: upsert Spec-N-Roll entry only; preserve unrelated MCP servers.
