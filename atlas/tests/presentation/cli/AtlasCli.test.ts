...
/**
 * Rejects view work when tests exercise commands that do not require serving artifacts.
 */
class UnreachableViewWorkflow implements ViewArtifactsWorkflow {
  /**
   * Rejects unexpected artifact serving during command-shell tests.
   *
   * @param request - Workspace request that should not be used by this test.
   * @param host - Host value that should not be used by this test.
   * @param port - Port value that should not be used by this test.
   * @param openBrowser - Browser launch setting that should not be used by this test.
   * @returns A rejected view promise.
   */
  public execute(
    _request: WorkspaceLoadingRequest,
    _host: string,
    _port: number,
    _openBrowser: boolean
  ): Promise<ArtifactServerLocation> {
    return Promise.reject(new Error('View was not expected.'));
  }
}
