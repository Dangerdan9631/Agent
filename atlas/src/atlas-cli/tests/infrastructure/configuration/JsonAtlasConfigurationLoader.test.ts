...
class TemporaryConfigurationDirectory {
  ...
  /**
   * Writes a JSON configuration document under the temporary directory.
   *
  ...
  public async write(document: unknown): Promise<string> {
    const configurationPath = join(this.directoryPath, 'atlas.config.json');
    await writeFile(configurationPath, `${JSON.stringify(document, undefined, 2)}\n`, 'utf8');
...
/**
 * Verifies JSON schema validation at the configuration infrastructure boundary.
 */
describe('JsonAtlasConfigurationLoader', () => {
  ...
  it('loads a valid canonical configuration', async () => {
    ...
    const configuration = await new JsonAtlasConfigurationLoader().load(configurationPath);
  ...
  it('rejects unrecognized configuration fields', async () => {
    ...
    await expect(new JsonAtlasConfigurationLoader().load(configurationPath)).rejects.toEqual(
  ...
  it('rejects duplicate module group IDs', async () => {
    ...
    await expect(new JsonAtlasConfigurationLoader().load(configurationPath)).rejects.toThrow(
      "Module group ID 'application' must be unique."
    );
  });
});
