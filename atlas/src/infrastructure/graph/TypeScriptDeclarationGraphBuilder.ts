...
class TypeScriptGraphState {
  ...
  private isModuleStatement(statement: ts.Statement): boolean {
    return ts.isFunctionDeclaration(statement) || ts.isVariableStatement(statement);
  }
  ...
  private registerSymbol(declaration: ts.Statement, nodeId: string): void {
    ...
    const nameNode = (
      declaration as
        ts.ClassDeclaration | ts.InterfaceDeclaration | ts.TypeAliasDeclaration | ts.EnumDeclaration
    ).name;
