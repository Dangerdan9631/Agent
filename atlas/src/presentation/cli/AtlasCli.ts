...
export class AtlasCli {
  ...
  private createProgram(): Command {
    ...
    program
      .name('atlas')
      .description('Understand and enforce TypeScript architecture.')
      .showHelpAfterError()
      .option(
        '--workspace <path>',
    ...
    program
      .command('view')
      .description('Serve generated artifacts locally.')
      .option('--host <host>')
      .option('--port <port>')
      .option('--open')
  ...
  /**
   * Starts a constrained local server for the selected workspace artifact root.
   *
   * @param rootCommand - Root Commander program carrying global options.
   * @param options - View command options parsed by Commander.
   * @returns A promise that resolves after local serving begins.
   */
  private async viewArtifacts(rootCommand: Command, options: AtlasViewOptions): Promise<void> {
    ...
    this.outputWriter.writeLine(`Serving Atlas artifacts at ${location.url}`);
  }

  /**
   * Parses one optional local-server port number.
   *
   * @param value - Raw command option value.
   * @returns Port within the TCP range, or the default Atlas view port.
   */
  private toPort(value: string | undefined): number {
    if (value === undefined) {
      return 4173;
    }
...
/**
 * Represents view-command options after Commander parsing.
 */
interface AtlasViewOptions {
  /**
   * Optionally overrides the local server host binding.
   */
  readonly host?: string;

  /**
   * Optionally overrides the local server TCP port.
   */
  readonly port?: string;

  /**
   * Requests opening the served URL in a browser when the runtime supports it.
   */
