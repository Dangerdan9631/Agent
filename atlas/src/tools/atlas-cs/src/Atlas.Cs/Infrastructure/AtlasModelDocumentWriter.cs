...
public sealed class AtlasModelDocumentWriter
{
    ...
    public AtlasModelDocumentWriter(string schemaDirectory)
    {
    ...
    /// <returns>Absolute path to `atlas-workspace.json`.</returns>
    public async Task<string> WriteAsync(string modelsDirectory, IReadOnlyList<AtlasModuleModel> models)
    {
        ...
        {
            var fileName = $"{Uri.EscapeDataString(model.Module.Id)}.atlas-module.json";
            var json = this.SerializeAndValidate(model, this.moduleSchema, $"module '{model.Module.Id}'");
            await File.WriteAllTextAsync(Path.Combine(modelsDirectory, fileName), json).ConfigureAwait(false);
            entries.Add(new AtlasWorkspaceManifestEntry(model.Module.Id, fileName));
        }

        var manifest = new AtlasWorkspaceManifest(1, entries);
        var manifestJson = this.SerializeAndValidate(manifest, this.workspaceSchema, "workspace manifest");
        var manifestPath = Path.Combine(modelsDirectory, "atlas-workspace.json");
    ...
    {
        var json = JsonSerializer.Serialize(value, this.options) + Environment.NewLine;
