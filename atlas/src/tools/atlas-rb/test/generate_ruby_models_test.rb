...
class GenerateRubyModelsTest < Minitest::Test
  ...
  def test_generates_gemless_root_application
    with_temp_directory("atlas-rb-root-") do |root|
      ...
      write_file(root, "atlas.config.json", JSON.pretty_generate(configuration_for(
      ...
      model = JSON.parse(File.read(File.join(File.dirname(manifest_path), manifest.dig("modules", 0, "modelPath")),
                                   encoding: "UTF-8"))
  ...
  def test_generates_gem_workspace
    with_temp_directory("atlas-rb-gems-") do |root|
      ...
      models = JSON.parse(File.read(manifest_path, encoding: "UTF-8")).fetch("modules")
      ...
      application = JSON.parse(File.read(File.join(File.dirname(manifest_path), application_path), encoding: "UTF-8"))
  ...
  def test_packaged_schemas_match_canonical_contracts
    ...
    %w[atlas.schema.json atlas-module.schema.json atlas-workspace.schema.json].each do |name|
      assert_equal File.binread(File.join(canonical_root, name)), File.binread(File.join(ruby_root, "schemas", name))
    end
  end
